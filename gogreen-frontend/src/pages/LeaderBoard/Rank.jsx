...
const Rank = ({ totalCarbonEmissions, setReload }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/api/leaderboard/submitScore`,
        {
          method: "POST",
          headers: {
  ...
  const fetchLeaderboardData = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/api/leaderboard/getScore`
      );
      const data = await response.json();
      console.log("Leaderbord get data :", data);
      const sortedEntries = [...data].sort(
        (a, b) => a.carbonFootprint - b.carbonFootprint
      );
  ...
  return (
    <div className={styles.leaderboardBox}>
      ...
      <div className={styles.leaderboardContainer}>
        <div className={styles.leaderboardCardHeading}>
          ...
          <p className={styles.carbonFootprintText}>
        ...
        {leaderboardEntries.map((entry) => (
          ...
          >
            ...
            <p className={styles.carbonFootprintText}>
              <span className={styles.carbonFootprintValue}>
